...
export function BankAccountManager({ userId, userRole }: BankAccountManagerProps) {
  ...
  const canManageBankAccounts = userRole === 'streamer' || userRole === 'admin';
  ...
  const handleInputChange = (field: keyof CreateBankAccountDto, value: string | boolean) => {
    setFormData(prev => ({
      ...prev,
      [field]: value,
    }));
        ...
        <div className="space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 mb-4">Add your first bank account to receive payments and withdrawals.</p>
              <Button
                onClick={() => setIsAddingAccount(true)}
                className="bg-indigo-600 hover:bg-indigo-700 text-white"
              >
