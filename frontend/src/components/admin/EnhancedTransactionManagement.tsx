...
interface Transaction {
  ...
  userId: string;
  userName: string;
  type: 'donation' | 'withdrawal' | 'refund' | 'fee' | 'transfer';
  amount: number;
  currency: string;
  status: 'pending' | 'completed' | 'failed' | 'disputed' | 'cancelled' | 'processing';
...
export function EnhancedTransactionManagement() {
  ...
  // Mock transaction data with enhanced fields
  ...
  useEffect(() => {
    ...
    if (disputeStatusFilter !== 'all') {
      filtered = filtered.filter(transaction => transaction.disputeStatus === disputeStatusFilter);
    }

    setFilteredTransactions(filtered);
  }, [transactions, searchTerm, statusFilter, typeFilter, disputeStatusFilter]);
  ...
  const handleDisputeResolution = (transactionId: string, resolution: string, notes?: string) => {
    console.log(`Resolving dispute for transaction ${transactionId} with resolution: ${resolution}`);
    setShowDisputeModal(false);
    // In real implementation, this would call the API
  };

  const handleManualAdjustment = (transactionId: string, amount: number, reason: string, notes?: string) => {
    console.log(`Making adjustment for transaction ${transactionId}: ${amount}, reason: ${reason}`);
  ...
  const getDisputeStatusBadge = (disputeStatus?: string) => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Enhanced Transaction Management</h1>
            <p className="text-sm text-gray-600">Comprehensive transaction management with dispute handling and manual adjustments</p>
          </div>
            ...
            <div className="mt-3">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">₫{selectedTransaction.processingFee.toLocaleString()}</p>
                </div>
                
                <div>
                  <label className="block text-sm font-medium text-gray-700">Net Amount</label>
                  <p className="mt-1 text-sm text-gray-900">₫{selectedTransaction.netAmount.toLocaleString()}</p>
                </div>
                ...
                {selectedTransaction.manualAdjustment && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Manual Adjustment</label>
                    <p className="mt-1 text-sm text-gray-900">₫{selectedTransaction.manualAdjustment.toLocaleString()}</p>
                  </div>
                )}
      ...
      {/* Enhanced Dispute Handling Modal */}
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <div className="mt-6 flex space-x-3">
                <button
                  onClick={() => {
                    const resolution = (document.getElementById('disputeResolution') as HTMLSelectElement).value;
                    const notes = (document.querySelector('textarea') as HTMLTextAreaElement).value;
                    handleDisputeResolution(selectedTransaction.id, resolution, notes);
                  }}
      ...
      {showAdjustmentModal && selectedTransaction && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="mt-6 flex space-x-3">
                <button
                  onClick={() => {
                    ...
                    const reasonInput = document.querySelector('input[type="text"]') as HTMLInputElement;
                    ...
                    handleManualAdjustment(selectedTransaction.id, amount, reason, notes);
                  }}
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Selected Transactions</label>
                  <p className="mt-1 text-sm text-gray-500">{selectedTransactions.length} transactions selected</p>
                </div>
              ...
              <div className="mt-6 flex space-x-3">
                <button
                  onClick={() => {
                    const action = (document.getElementById('bulkAction') as HTMLSelectElement).value;
                    const reason = (document.querySelector('input[type="text"]') as HTMLInputElement).value;
                    ...
                    handleBulkAction(action, reason);
                  }}
